...
import 'partner_business_stats_page.dart';
...
class PartnerDashboardStats {
  ...
  /// PartnerDashboardResponse.ticketsSoldOnline/Offline (backend).
    ...
    FutureProvider.autoDispose<({double total, double online, double offline})>(
      (ref) async {
        ...
        for (final b in bookings) {
          ...
          // partielle). Même formule que PartnerTransactionResponse/PartnerBookingItem.grossAmount,
          // pour que ce chiffre reste toujours aligné avec les pages Transactions et Mes
          // réservations.
                  ...
                  children: [
                    ...
                    GestureDetector(
                      onTap: () => Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => const PartnerBusinessStatsPage(),
                        ),
                      ),
                      child: Container(
                        width: double.infinity,
...
/// Carte revenus "Depuis toujours" — même structure visuelle que la carte mensuelle
/// (Consumer + _monthlyRevenueProvider un peu plus bas), mais lit directement `stats`
/// (déjà chargé par _partnerDashboardProvider), sans provider ni filtre de période.
